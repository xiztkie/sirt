<x-app-layout :title="$title" :active="$active" :subtitle="$subtitle ?? ''">
    <div class="card bg-white border border-base-200 shadow-sm p-6">
        <div class="mb-6">
            <h3 class="text-xl font-semibold">Master Data Kecamatan</h3>
            <p class="text-sm text-base-content/60">
                Kelola daftar Kecamatan
            </p>
        </div>
        ...
        <div class="flex flex-col md:flex-row justify-between gap-4 mb-6">
            <div class="flex items-center gap-2">
                <button type="button" class="btn btn-success" aria-haspopup="dialog" aria-expanded="false"
                    aria-controls="sync-kecamatan-modal" data-overlay="#sync-kecamatan-modal">
            ...
            <form id="searchForm" action="{{ route('datawilayah.kecamatan') }}" method="GET">
                <div class="relative w-full">
                    <input type="text" name="search" value="{{ $search ?? '' }}"
                        placeholder="Cari kecamatan..." class="input input-bordered w-full pl-10" id="searchInput">
        ...
        <div class="overflow-x-auto rounded-xl border border-gray-200">
            <table class="table-xs table table-zebra">
                <thead class="bg-base-100 text-sm">
                    <tr>
                        ...
                        <th class="py-4 text-center">Kode Kecamatan</th>
                ...
                <tbody>
                    ...
                    @foreach ($kecamatan as $item)
                        <tr class="row-hover">
                            ...
                            <td><a href="{{ route('datawilayah.kelurahan', ['search' => $item->code]) }}"
         ...
         <div class="p-4">
            @if(method_exists($kecamatan, 'links'))
                {{ $kecamatan->appends(['search' => request('search')])->links() }}
            @endif
        </div>

    </div>

    <div id="sync-kecamatan-modal"
        class="overlay modal modal-middle hidden overlay-open:opacity-100 bg-black/70 overlay-open:duration-300"
        role="dialog" tabindex="-1">
        <div class="modal-dialog modal-dialog-sm modal-dialog-centered">
            <form id="syncKecamatanForm" action="{{ route('datawilayah.sync.kecamatan') }}" method="POST"
                class="modal-content" aria-describedby="sync-kecamatan-desc">
                @csrf
                <div class="modal-header bg-yellow-100 rounded-t-md flex items-center gap-3">
                    <div>
                        <h3 class="modal-title text-lg font-bold">Sync Data Kecamatan</h3>

                    </div>

                    <button type="button" class="btn btn-text btn-circle btn-sm absolute inset-e-3 top-3"
                        aria-label="Close" data-overlay="#sync-kecamatan-modal">
                        <span class="icon-[tabler--x] size-5"></span>
                    </button>
                </div>

                <div id="sync-kecamatan-desc" class="modal-body p-6">
                    <div class="flex items-start gap-4">
                        <div>
                            <p class="text-sm font-medium mb-2">Anda akan melakukan sinkronisasi data kecamatan.
                            </p>
                ...
                <div class="modal-footer border-t border-gray-200 pt-4 flex justify-end gap-2">
                    <button id="syncKecamatanCancel" type="button" class="btn btn-soft btn-secondary"
                        data-overlay="#sync-kecamatan-modal">Batal</button>
                    <button id="syncKecamatanSubmit" type="submit" class="btn btn-primary" aria-busy="false">
                        <span class="inline-flex items-center gap-2">
                            <span class="hidden" id="syncKecamatanSpinner"><svg xmlns="http://www.w3.org/2000/svg"
                            ...
                            <span id="syncKecamatanLabel">Ya, ambil data</span>
                        </span>
                    </button>
                </div>
            </form>
    ...
    <script>
        (function() {
            const form = document.getElementById('syncKecamatanForm');
            if (!form) return;

            const submitBtn = document.getElementById('syncKecamatanSubmit');
            const cancelBtn = document.getElementById('syncKecamatanCancel');
            const spinner = document.getElementById('syncKecamatanSpinner');
            const label = document.getElementById('syncKecamatanLabel');
